...
module.exports = {
  name: "sunucu-log",
  description: "Sunucu log sistemini açmak veya kapatmak için kullanılır.",
  guildOnly: true,
  async execute(message, args, bot) {
    ...
    if (args[0] === "aç") {
      if (guildData && guildData.guildLog == true)
        return message.reply({
    ...
    if (args[0] === "kapat") {
      if (guildData && guildData.guildLog == false)
        return message.reply({
    ...
    if (args[0] === "log-sıfırla") {
      if (guildData && guildData.guildChannelID) {
        await database
          .findOneAndUpdate(
            { guildID: message.guild.id },
            { $unset: { guildChannelID: "" } },
            { upsert: true }
          )
          .then(() => {
            message.reply({
