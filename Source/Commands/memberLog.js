...
module.exports = {
  name: "member-log",
  description: "Member log sistemini açmak veya kapatmak için kullanılır.",
  guildOnly: true,
  async execute(message, args, bot) {
    ...
    if (args[0] === "aç") {
      if (guildData && guildData.memberLog == true)
        return message.reply({
    ...
    if (args[0] === "kapat") {
      if (guildData && guildData.memberLog == false)
        return message.reply({
    ...
    if (args[0] === "log-ayarla") {
      ...
      await database
        .findOneAndUpdate(
          { guildID: message.guild.id },
          { $set: { memberChannelID: channel.id } },
          { upsert: true }
        )
        .then(() => {
          message.reply({
    ...
    if (args[0] === "log-sıfırla") {
      if (guildData && guildData.memberChannelID) {
        await database
          .findOneAndUpdate(
            { guildID: message.guild.id },
            { $unset: { memberChannelID: "" } },
            { upsert: true }
          )
          .then(() => {
            message.reply({
