...
module.exports = {
  name: "rol-log",
  description: "Rol log sistemini açmak veya kapatmak için kullanılır.",
  guildOnly: true,
  async execute(message, args, bot) {
    ...
    if (args[0] === "aç") {
      if (guildData && guildData.roleLog == true)
        return message.reply({
    ...
    if (args[0] === "kapat") {
      if (guildData && guildData.roleLog == false)
        return message.reply({
    ...
    if (args[0] === "log-sıfırla") {
      if (guildData && guildData.roleChannelID) {
        await database
          .findOneAndUpdate(
            { guildID: message.guild.id },
            { $unset: { roleChannelID: "" } },
